...
后端逻辑完全一致：先用 `code` 解析出剧本，再用「询问」**向量化**后在该剧本手册内
做**向量检索**，最后由 LLM 把命中内容合成一条**带引用出处**的答案。检索范围被
`code` 严格限定在该剧本手册内，不会跨剧本串味。
...
| `answer` | string | **LLM 合成的、可直接照着念的答案**，含括号标注的出处 |
...
| `tookMs` | integer | 端到端耗时（毫秒，含向量化 + 检索 + 生成） |
...
- **防抖**：用户对输入框做 `debounce`（300~500ms）后再自动提问，减少 LLM 额度消耗。
- **流式/加载态**：`tookMs` 通常 1~3 秒，按钮需有 loading 态，禁止重复提交。
