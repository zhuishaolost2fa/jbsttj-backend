...
async def get_dm_guide_qa_titles(
    id_or_code: str = Path(description="剧本 UUID 或业务编码"),
    scripts: ScriptService = Depends(get_script_service),
    service: DMGuideService = Depends(get_dm_guide_service),
) -> QATitleChain:
    script = await scripts.get_script(id_or_code)
    return await service.qa_title_chain(
...
@ask_router.get(
    ...
    description=(
        ...
        "返回结构同 `GET /scripts/{id_or_code}/dm-guide/qa-titles`。"
    ),
)
async def list_qa_titles_by_name(
    ...
    if not resolved:
        if not name:
            raise ValidationError(
                "title 与 code 至少传一个", code="title_or_code_required"
            )
        # 与入库时 script_dm_code 的派生规则一致：slugify(剧本名)
