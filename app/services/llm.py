...
# 提示词
# ============================================================
_QA_SYSTEM_PROMPT = """你是一名资深的剧本杀主持人（DM）培训师。
你的任务是把《主持人手册》的片段，改写成主持人在**实际带本过程中会真正查询**的问答对。

硬性要求：
1. 问题必须能仅凭所给片段回答，严禁引入片段之外的任何信息，严禁编造。
2. 问题要用主持人的口吻提问，例如「搜证阶段每人能搜几次」，
   而不是「本段讲了什么」这类无检索价值的元问题。
3. 问题中不要出现「本文」「该片段」「上述」等指代词——
   这些问答对会被单独检索命中，脱离上下文后指代词就失效了。
4. 答案要完整、可直接照着执行，保留原文中的数字、时间、人名、道具名。
5. category 从以下枚举中选一个最贴切的：
   rule（规则流程）/ clue（线索物证）/ character（角色人设）/
   timeline（时间线剧情）/ host_tip（主持技巧提醒）/ general（其他）。
6. 若片段内容过于零碎（如纯目录、纯页眉、无实质信息），返回空数组 []。
...
def build_qa_user_prompt(
    ...
    *,
    ...
    lines.append(
        ...
        f"每个片段最多 {qa_per_chunk} 条，内容单薄的片段可以少生成或不生成。"
    )
    lines.append("")
